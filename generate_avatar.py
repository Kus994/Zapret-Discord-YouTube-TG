"""Генерация аватарки для KUS Pro v1.0.0"""
...
for y in range(SIZE):
    for x in range(SIZE):
        ...
        alpha = max(0, min(255, int(255 * (1 - dist * 1.5))))
        r = int(10 + 5 * dist)
        g = int(14 + 8 * dist)
        b = int(20 + 15 * dist)
        img.putpixel((x, y), (r, g, b, 255))

# 2. Круглое свечение в центре
...
for i in range(6):
    ...
    draw.ellipse([center_x - r, center_y - r, center_x + r, center_y + r], 
                 outline=(0, 255, 136, alpha), width=3)
...
for angle in range(0, 360, 30):
    ...
    draw.ellipse([px - 4, py - 4, px + 4, py + 4], fill=(0, 255, 136, 180))
...
print(f"ICO сохранена: {ico_path}")
